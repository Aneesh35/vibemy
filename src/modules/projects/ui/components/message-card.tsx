...
const FragmentCard = ({ fragment, isActiveFragment, onFragmentClick }: FragmentProps) => {
    return (
        <button className={cn("flex items-start gap-2 border rounded-lg bg-muted w-fit p-3 hover:bg-secondary transition-colors", isActiveFragment && "bg-primary-foreground border-primary hover:bg-primary")}
...
const MessageCard = ({ content, role, fragment, createdAt, isActiveFragment, type, onFragmentClick }: MessageCardProps) => {
    if (role == "ASSISTANT") {
        return (
            <AssistantMessage
                content={content}
